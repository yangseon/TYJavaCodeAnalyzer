...
public final class ObjectCreationExpr extends Expression implements NodeWithTypeArguments<ObjectCreationExpr>, NodeWithType<ObjectCreationExpr, ClassOrInterfaceType>, NodeWithArguments<ObjectCreationExpr>, NodeWithOptionalScope<ObjectCreationExpr> {
    ...
    public ObjectCreationExpr(final Range range, final Expression scope, final ClassOrInterfaceType type, final NodeList<Type> typeArguments, final NodeList<Expression> arguments, final NodeList<BodyDeclaration<?>> anonymousClassBody) {
        ...
        if (this.arguments != null && this.arguments.isEmpty() == false) {
       	...
       	BodyDeclaration<?> firstBodyDeclaration = null, lastBodyDeclaration = null;
 
        if (this.anonymousClassBody != null && this.anonymousClassBody.isEmpty() == false) {
        	for(BodyDeclaration<?> bd:this.anonymousClassBody) {
        		if(bd != null && bd.getRange().isPresent()) {
	        		this.complexity += (bd.getComplexity() -1);
	        		this.codeLoc += bd.getCodeLoc();
	        		
	        		if(firstBodyDeclaration != null && lastBodyDeclaration != null) {
	            		// contains with arg
	            		if(firstBodyDeclaration.getBeginLine() <= bd.getBeginLine()
            					&& lastBodyDeclaration.getEndLine() >= bd.getEndLine()) {
	            			this.codeLoc -= bd.getCodeLoc();
	        	        }
	            		else if(firstBodyDeclaration.getBeginLine() == bd.getEndLine()) {
	            			this.codeLoc--;
	            		} else if(lastBodyDeclaration.getEndLine() == bd.getBeginLine()) {
	            			this.codeLoc--;
	            		}
	            	} else {
	        		...
	        		if(firstBodyDeclaration == null) {
	        			firstBodyDeclaration = bd;
		     		} else if(firstBodyDeclaration.getBeginLine() > bd.getBeginLine()) {
		     			firstBodyDeclaration = bd;
		     		}
		     		
		     		if(lastBodyDeclaration == null) {
		     			lastBodyDeclaration = bd;
		     		} else if(lastBodyDeclaration.getEndLine() < bd.getEndLine()) {
		     			lastBodyDeclaration = bd;
		     		}
	        	}
        	}
        	
        	if(firstBodyDeclaration != null && firstBodyDeclaration.getRange().isPresent()) {
	        	if(firstNode == null) {
	        		firstNode = firstBodyDeclaration;
	        	} else {
	        		if(firstNode.getBeginLine() > firstBodyDeclaration.getBeginLine()) {
	        			firstNode = firstBodyDeclaration;
	        		}
	        	}
        	}
        	if(lastBodyDeclaration != null && lastBodyDeclaration.getRange().isPresent()) {
	            if(lastNode == null) {
	            	lastNode = lastBodyDeclaration;
	            } else {
	            	if(lastNode.getEndLine() < lastBodyDeclaration.getEndLine()) {
	            		lastNode = lastBodyDeclaration;
	        		}
	            }
        	}
