...
public final class VariableDeclarationExpr extends Expression implements NodeWithFinalModifier<VariableDeclarationExpr>, NodeWithAnnotations<VariableDeclarationExpr>, NodeWithVariables<VariableDeclarationExpr> {
    ...
    public VariableDeclarationExpr(final Range range, final EnumSet<Modifier> modifiers, final NodeList<AnnotationExpr> annotations, final NodeList<VariableDeclarator> variables) {
        ...
        VariableDeclarator firstVar = null, lastVar = null;
        
        
        if (this.variables != null && this.variables.isEmpty() == false) {
             for(VariableDeclarator var:this.variables) {
             	if (var != null && var.getRange().isPresent()) {
 	            	this.codeLoc += var.getCodeLoc();
 	            	this.complexity += (var.getComplexity() - 1);
 	            	
	            	if(firstVar != null && lastVar != null) {
	            		// contains with val
	            		if(firstVar.getBeginLine() <= var.getBeginLine()
            					&& lastVar.getEndLine() >= var.getEndLine()) {
	            			this.codeLoc -= var.getCodeLoc();
	        	        }
	            		else if(firstVar.getBeginLine() == var.getEndLine()) {
	            			this.codeLoc--;
	            		} else if(lastVar.getEndLine() == var.getBeginLine()) {
	            			this.codeLoc--;
	            		}
	            	} else {
	            		
	            		if(firstNode != null && lastNode != null) {
	            			if(firstNode.getBeginLine() <= var.getBeginLine()
	            					&& lastNode.getEndLine() >= var.getEndLine()) {
		            			this.codeLoc -= var.getCodeLoc();
		        	        }
		            		else if(firstNode.getBeginLine() == var.getEndLine()) {
		            			this.codeLoc--;
		            		} else if(lastNode.getEndLine() == var.getBeginLine()) {
		            			this.codeLoc--;
		            		}
	            		}
	            	}
 	        		...
 	        		if(firstVar == null) {
 	         		...
 	         		if(lastVar == null) {
 	        			...
 	        			if(lastVar.getEndLine() < var.getEndLine()) {
 	        				lastVar = var;
 	        			}
 	        		}
 	            }
             ...
             if(firstVar != null && firstVar.getRange().isPresent()) {
 	            if(firstNode == null) {
 	        		firstNode = firstVar;
 	        	} 
             }
             if(lastVar != null && lastVar.getRange().isPresent()) {  
 	            if(lastNode == null) {
 	            	lastNode = lastVar;
 	            } 
             }
