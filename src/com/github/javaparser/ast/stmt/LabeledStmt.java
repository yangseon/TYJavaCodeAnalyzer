...
public final class LabeledStmt extends Statement {
    ...
    public LabeledStmt(Range range, final SimpleName label, final Statement statement) {
		...
		if (this.statement != null && this.statement.getRange().isPresent()) {
			this.codeLoc += this.statement.getCodeLoc();
		    this.complexity += (this.statement.getComplexity() - 1);

		    if(firstNode == null) { 
             	firstNode = this.statement;
             }
             if(lastNode == null) {
             	lastNode = this.statement;
             }
		}
