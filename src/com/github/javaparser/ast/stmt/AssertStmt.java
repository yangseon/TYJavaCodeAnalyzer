...
public final class AssertStmt extends Statement {
    ...
    public AssertStmt(final Range range, final Expression check, final Expression message) {
        super(range);
        // AssertStmt's codeLoc equals loc
        setCheck(check);
        setMessage(message);
        ...
        if (this.check != null && this.check.getRange().isPresent()) {
        	this.codeLoc += this.check.getCodeLoc();
        	this.complexity += (this.check.getComplexity() -1);
            if(firstNode == null) { 
            	firstNode = this.check;
            }
            if(lastNode == null) {
            	lastNode = this.check;
            }
        ...
        if (this.message != null && this.message.getRange().isPresent()) {
        	this.codeLoc += this.message.getCodeLoc();
        	this.complexity += (this.message.getComplexity() -1);
            if(lastNode != null && lastNode.getRange().isPresent()) {
				if(lastNode.getEndLine() == this.message.getBeginLine()) {
					this.codeLoc--;
				}
			}
            
            if(firstNode == null) {
        		firstNode = this.message;
        	} else {
        		if(firstNode.getBeginLine() > this.message.getBeginLine()) {
        			firstNode = this.message;
        		}
        	}
        	...
        	if(lastNode == null) {
            	lastNode = this.message;
            } else {
            	if(lastNode.getEndLine() < this.message.getEndLine()) {
            		lastNode = this.message;
        		}
            }
