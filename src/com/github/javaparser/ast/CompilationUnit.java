...
public final class CompilationUnit extends Node {
    ...
    public CompilationUnit(Range range, PackageDeclaration packageDeclaration, NodeList<ImportDeclaration> imports, NodeList<TypeDeclaration<?>> types, ModuleDeclaration module) {
        ...
        /**
         * Excludes analysis on modules, then below code do nothing
         */
        if (this.module != null && this.module.getRange().isPresent()) {
        	this.codeLoc += this.module.getCodeLoc();
        	this.complexity += (this.module.getComplexity() - 1);

        	 if(firstNode == null) { 
             	firstNode = this.module;
             }
             if(lastNode == null) {
             	lastNode = this.module;
             }
        }
        /**
         * above code  do nothing
         */
        
        if (this.packageDeclaration != null && this.packageDeclaration.getRange().isPresent()) {
        	this.codeLoc += this.packageDeclaration.getCodeLoc();
        	
        	 /*
        	  * PackageDeclaration dose not have complexity
        	  */
        	//this.complexity += (this.packageDeclaration.getComplexity() - 1);

        	if(lastNode != null && lastNode.getRange().isPresent()) {
	        	if(lastNode.getEndLine() == this.packageDeclaration.getBeginLine()) {
	            	this.codeLoc--;
	            }
        	}
        	if(firstNode == null) {
        		firstNode = this.packageDeclaration;
        	} else {
        		if(firstNode.getBeginLine() > this.packageDeclaration.getBeginLine()) {
        			firstNode = this.packageDeclaration;
        		}
        	}
        	...
        	if(lastNode == null) {
            	lastNode = this.packageDeclaration;
            } else {
            	if(lastNode.getEndLine() < this.packageDeclaration.getEndLine()) {
            		lastNode = this.packageDeclaration;
        		}
            }
        }
        
        ImportDeclaration firstImptD = null, lastImptD = null;
       
    
        if (this.imports != null && this.imports.isEmpty() == false) {
            for(ImportDeclaration impt:this.imports) {
            	if(impt != null && impt.getRange().isPresent()) {
	            	this.codeLoc += impt.getCodeLoc();
	            	/*
	            	 * ImportDeclaration dose not have complexity
	            	 */
	            	//this.complexity += (impt.getComplexity() - 1);

	            	if(firstImptD != null && lastImptD != null) {
	            		// contains with ImportDeclaration
	            		if(firstImptD.getBeginLine() <= impt.getBeginLine()
            					&& lastImptD.getEndLine() >= impt.getEndLine()) {
	            			this.codeLoc -= impt.getCodeLoc();
	        	        }
	            		else if(firstImptD.getBeginLine() == impt.getEndLine()) {
	            			this.codeLoc--;
	            		} else if(lastImptD.getEndLine() == impt.getBeginLine()) {
	            			this.codeLoc--;
	            		}
	            	} else {
	            		
	            		if(firstNode != null && lastNode != null) {
	            			if(firstNode.getBeginLine() <= impt.getBeginLine()
	            					&& lastNode.getEndLine() >= impt.getEndLine()) {
		            			this.codeLoc -= impt.getCodeLoc();
		        	        }
		            		else if(firstNode.getBeginLine() == impt.getEndLine()) {
		            			this.codeLoc--;
		            		} else if(lastNode.getEndLine() == impt.getBeginLine()) {
		            			this.codeLoc--;
		            		}
	            		}
	            	}
	        		...
	        		if(firstImptD == null) {
	         		...
	         		if(lastImptD == null) {
	        			...
	        			if(lastImptD.getEndLine() < impt.getEndLine()) {
	        				lastImptD = impt;
	        			}
	        		}
	            }
            ...
            if(firstImptD != null && firstImptD.getRange().isPresent()) {
	            if(firstNode == null) {
	        		firstNode = firstImptD;
	        	} else {
	        		if(firstNode.getBeginLine() > firstImptD.getBeginLine()) {
	        			firstNode = firstImptD;
	        		}
	        	}
            }
            if(lastImptD != null && lastImptD.getRange().isPresent()) {
	            if(lastNode == null) {
	            	lastNode = lastImptD;
	            } else {
	            	if(lastNode.getEndLine() < lastImptD.getEndLine()) {
	            		lastNode = lastImptD;
	        		}
	            }
            }
	    ...
	    TypeDeclaration<?> firstTypeD = null, lastTypeD = null;
	      
	    if (this.types != null && this.types.isEmpty() == false) {
	    	for(TypeDeclaration<?> t: this.types) {
				if(t != null && t.getRange().isPresent()) {
					this.codeLoc += t.getCodeLoc();
					this.complexity += (t.getComplexity() - 1);
			
					if(firstTypeD != null && lastTypeD != null) {
	            		// contains with TypeDeclaration
	            		if(firstTypeD.getBeginLine() <= t.getBeginLine()
            					&& lastTypeD.getEndLine() >= t.getEndLine()) {
	            			this.codeLoc -= t.getCodeLoc();
	        	        }
	            		else if(firstTypeD.getBeginLine() == t.getEndLine()) {
	            			this.codeLoc--;
	            		} else if(lastTypeD.getEndLine() == t.getBeginLine()) {
	            			this.codeLoc--;
	            		}
	            	} else {
	            		
	            		if(firstNode != null && lastNode != null) {
	            			if(firstNode.getBeginLine() <= t.getBeginLine()
	            					&& lastNode.getEndLine() >= t.getEndLine()) {
		            			this.codeLoc -= t.getCodeLoc();
		        	        }
		            		else if(firstNode.getBeginLine() == t.getEndLine()) {
		            			this.codeLoc--;
		            		} else if(lastNode.getEndLine() == t.getBeginLine()) {
		            			this.codeLoc--;
		            		}
	            		}
	            	}
	        		...
	        		if(firstTypeD == null) {
	         		...
	         		if(lastTypeD == null) {
	        			...
	        			if(lastTypeD.getEndLine() < t.getEndLine()) {
	        				lastTypeD = t;
	        			}
	        		}
				}
			...
			if(firstTypeD != null && firstTypeD.getRange().isPresent()) {
				if(firstNode == null) {
					firstNode = firstTypeD;
				} else {
					if(firstNode.getBeginLine() > firstTypeD.getBeginLine()) {
						firstNode = firstTypeD;
					}
				}
			}
			
			if(lastTypeD != null && lastTypeD.getRange().isPresent()) {
				if(lastNode == null) {
					lastNode = lastTypeD;
				} else {
					if(lastNode.getEndLine() < lastTypeD.getEndLine()) {
						lastNode = lastTypeD;
					}
				}
			}
