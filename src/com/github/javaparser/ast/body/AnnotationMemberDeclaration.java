...
public final class AnnotationMemberDeclaration extends BodyDeclaration<AnnotationMemberDeclaration> implements NodeWithJavadoc<AnnotationMemberDeclaration>, NodeWithSimpleName<AnnotationMemberDeclaration>, NodeWithType<AnnotationMemberDeclaration, Type>, NodeWithPublicModifier<AnnotationMemberDeclaration>, NodeWithAbstractModifier<AnnotationMemberDeclaration> {
    ...
    public AnnotationMemberDeclaration(Range range, EnumSet<Modifier> modifiers, NodeList<AnnotationExpr> annotations, Type type, SimpleName name, Expression defaultValue) {
        ...
        if (this.defaultValue != null && this.defaultValue.getRange().isPresent()) {
        	this.codeLoc += this.defaultValue.getCodeLoc();
        	this.complexity += (this.defaultValue.getComplexity() - 1);
        	
        	if(this.defaultValue.getComment().isPresent() == true) {
        		this.commentLoc += this.defaultValue.getComment().get().getLoc();
        	}
        
	        if(this.defaultValue.getOrphanComments() != null && this.defaultValue.getOrphanComments().isEmpty() == false) {
	    		for(Comment comm:this.defaultValue.getOrphanComments()) {
	    			this.commentLoc += comm.getLoc();
	    		}
	    	}
	        
        	if(firstNode == null) { 
             	firstNode = this.defaultValue;
            }
            if(lastNode == null) {
             	lastNode = this.defaultValue;
            }
        }
