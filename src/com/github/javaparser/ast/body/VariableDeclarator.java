...
public final class VariableDeclarator extends Node implements NodeWithType<VariableDeclarator, Type>, NodeWithSimpleName<VariableDeclarator> {
    ...
    public VariableDeclarator(Range range, Type type, SimpleName name, Expression initializer) {
        ...
        long orgCodeLoc = this.setCodeLoc();

        Node firstNode = null, lastNode = null;
        
        if (this.initializer != null && this.initializer.getRange().isPresent()) {
        	this.codeLoc += this.initializer.getCodeLoc();
        	this.complexity += (this.initializer.getComplexity() - 1);
        	
        	if(firstNode == null) { 
             	firstNode = this.initializer;
            }
            if(lastNode == null) {
             	lastNode = this.initializer;
            }
        }
    ...
    protected long setCodeLoc() {
        ...
        if(firstNode != null && lastNode != null) {
        	this.codeLoc = lastNode.getEndLine() - firstNode.getBeginLine() + 1;
        } else {
