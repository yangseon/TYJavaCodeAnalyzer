...
public final class FieldDeclaration extends BodyDeclaration<FieldDeclaration> implements NodeWithJavadoc<FieldDeclaration>, NodeWithVariables<FieldDeclaration>, NodeWithAccessModifiers<FieldDeclaration>, NodeWithStaticModifier<FieldDeclaration>, NodeWithFinalModifier<FieldDeclaration> {
    ...
    public FieldDeclaration(Range range, EnumSet<Modifier> modifiers, NodeList<AnnotationExpr> annotations, NodeList<VariableDeclarator> variables) {
        ...
        VariableDeclarator firstVarD = null, lastVarD = null;
        ...
        if (this.variables != null) {
            for(VariableDeclarator var:this.variables) {
            	if (var != null && var.getRange().isPresent()) {
	            	...
	            	if(firstVarD != null && lastVarD != null) {
	            		// contains with arg
	            		if(firstVarD.getBeginLine() <= var.getBeginLine()
            					&& lastVarD.getEndLine() >= var.getEndLine()) {
	            			this.codeLoc -= var.getCodeLoc();
	        	        }
	            		else if(firstVarD.getBeginLine() == var.getEndLine()) {
	            			this.codeLoc--;
	            		} else if(lastVarD.getEndLine() == var.getBeginLine()) {
	            			this.codeLoc--;
	            		}
	            	} else {
	        		...
	        		if(firstVarD == null) {
	         		...
	         		if(lastVarD == null) {
	        			...
	        			if(lastVarD.getEndLine() < var.getEndLine()) {
	        				lastVarD = var;
	        			}
	        		}
            ...
            if(firstVarD != null && firstVarD.getRange().isPresent()) {
	            if(firstNode == null) {
	        		firstNode = firstVarD;
	        	} 
            }
            if(lastVarD != null && lastVarD.getRange().isPresent()) {
	            if(lastNode == null) {
	            	lastNode = lastVarD;
	            } 
            }
