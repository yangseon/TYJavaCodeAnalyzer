...
public final class InitializerDeclaration extends BodyDeclaration {
    ...
    public InitializerDeclaration(int beginLine, int beginColumn, int endLine, int endColumn, JavadocComment javaDoc, boolean isStatic, BlockStmt block, List<Comment> comments) {
        super(beginLine, beginColumn, endLine, endColumn, null, javaDoc, comments);
        this.isStatic = isStatic;
        this.block = block;
           
        if(block != null) {
	     	this.codeLoc = this.block.getCodeLoc();
    		if(beginLine < this.block.getBeginLine()) this.codeLoc++;
    		if(endLine > this.block.getEndLine()) this.codeLoc++;
    		
	     	this.loc = block.getLoc();
        }
