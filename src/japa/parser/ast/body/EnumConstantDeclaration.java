...
public final class EnumConstantDeclaration extends BodyDeclaration {
    ...
    public EnumConstantDeclaration(int beginLine, int beginColumn, int endLine, int endColumn, JavadocComment javaDoc, List<AnnotationExpr> annotations, String name, List<Expression> args, List<BodyDeclaration> classBody, List<Comment> comments) {
        ...
        if(classBody != null) {
        	int firstBodyLine = endLine, lastBodyLine = beginLine;
  			for(BodyDeclaration b:classBody) {
  				this.codeLoc += b.getCodeLoc();
				if(firstBodyLine > b.getBeginLine()) {
					firstBodyLine = b.getBeginLine();
				}
