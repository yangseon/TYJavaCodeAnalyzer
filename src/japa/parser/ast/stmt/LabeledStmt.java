...
public final class LabeledStmt extends Statement {
    ...
    public LabeledStmt(int beginLine, int beginColumn, int endLine, int endColumn, String label, Statement stmt) {
        ...
        if(this.stmt != null) {        	
    		this.codeLoc += stmt.getCodeLoc();
    		if(beginLine < stmt.getBeginLine()) {
    			this.codeLoc++;
    		}
    		if(endLine > stmt.getEndLine()) {
    			this.codeLoc++;
    		}    	
    		this.complexity += (this.stmt.getComplexity() - 1);
        }
//        System.out.println("LabeledStmt ("+beginLine+","+ endLine+") codeLoc="+this.codeLoc+" complexity="+this.complexity);
    }
