...
public final class SwitchStmt extends Statement {
    ...
    public SwitchStmt(int beginLine, int beginColumn, int endLine, int endColumn, Expression selector, List<SwitchEntryStmt> entries) {
        ...
        //ysryu
        this.codeLoc = 0;
     	int lastEndLine = 0, firstBeineLine = endLine;
        if(this.entries != null) {        	
        	for(Statement e:entries) {
        		this.codeLoc += e.getCodeLoc();
        		if(lastEndLine == e.getBeginLine()) {
        			this.codeLoc--;
        		} else {
        			lastEndLine = e.getEndLine();
        		}
        		if(firstBeineLine > e.getBeginLine()) {
        			firstBeineLine = e.getBeginLine();
        		}
        ...
        if(lastEndLine < endLine) {
        	this.codeLoc++;
        }
        if(firstBeineLine > beginLine) {
        	this.codeLoc++;
        }
        if(this.selector != null) {
        	this.complexity += (this.selector.getComplexity() - 1);
        }
//        System.out.println("SwitchStmt ("+beginLine+","+ endLine+") complexity="+this.complexity);
    }
