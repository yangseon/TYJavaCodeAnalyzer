...
public final class CompilationUnit extends Node {
    ...
    public CompilationUnit(int beginLine, int beginColumn, int endLine, int endColumn, PackageDeclaration pakage, List<ImportDeclaration> imports, List<TypeDeclaration> types, List<Comment> comments) {
        ...
        //ysryu
        int commentBegineLine = beginLine;
        this.commentLoc = 0;
        if(comments != null) {
	    	for(Comment c:comments) {
	    		this.commentLoc += c.getLoc();
	    		if(commentBegineLine > c.getBeginLine()) {
	    			commentBegineLine = c.getBeginLine();
	    		}
	    	}
	    	this.loc = endLine - commentBegineLine + 1;
	    } else {
            this.loc = endLine - beginLine + 1;
	    }
        
    	this.codeLoc = 0;
    	if(this.pakage != null) {
    		codeLoc += pakage.getCodeLoc();
    	}
    	if(this.imports != null) {
	    	for(ImportDeclaration i:imports) {
	    		codeLoc += i.getCodeLoc();    		
	    	}
    	}

    	List<BodyDeclaration> mList;
    	List<AnnotationExpr> aList;
    	if(types != null) {
	    	for(TypeDeclaration t:types) {
	    		if(t.getBeginLine() != t.getEndLine() )
	    			this.codeLoc += 2;
	    		else
	    			this.codeLoc += 1;
	    		
	    		aList = t.getAnnotations();
	    		if(aList != null) {
	    			for(AnnotationExpr a:aList) {
	    				codeLoc += a.getCodeLoc();
	    			}
	    		}
	    		
	    		mList = t.getMembers();
	    		if(mList != null) {
		    		for(BodyDeclaration m:mList) {
			    		codeLoc += m.getCodeLoc();
			    		aList = m.getAnnotations();
			    		if(aList != null) {
			    			for(AnnotationExpr a:aList) {
			    				codeLoc += a.getCodeLoc();
			    			}
